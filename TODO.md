...
- **`cens_factor` rebuilt as a long-format shared-factor model — RESOLVED
  ...
  factor. **Shared factor SD 0.54 [0.33, 0.76]** (the coupling is real).
  Reworked the `cens_factor` branches of `.fit_group_model()` +
  `.predict_and_merge()` (+ new `.predict_factor_long()`); the reshape is fully
  contained in the brms internals, `cens`/`rescor_mi` untouched. Pinned by a
  `VarCorr` assertion in `test-impute-smoke.R`. Full B.S01 benchmark (2000 iter,
  3 chains serial, `adapt_delta = 0.95` default) confirms it: R̂ **1.006**, 35
  divergences (best convergence of the three), RMSE **0.431** — between `cens`
  (0.533) and `rescor_mi` (0.247), with calibrated cov90 0.89.
