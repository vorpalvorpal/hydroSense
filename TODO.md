...
an escape hatch: retain an analyte that fails the frequency screen if
any detected concentration exceeds a potency-based threshold (e.g. a
fraction of its HC05 / guideline value). Lives in `R/prescreen.R`.
...
- **Probabilistic NO3-N hardness weighting** (`R/paf.R:22`, `:190`).
  Replace the hard hardness-class cutoff (soft/mod/hard SSDs) with a
  smooth log-normal weighting of the three class PAFs:
  `p_soft = plnorm(30, log(h), s)`,
  `p_hard = 1 - plnorm(150, log(h), s)`, `p_mod = 1 - p_soft - p_hard`,
  `s = sqrt(log(1+cv^2))`, default `cv = 0.05`.
  `ExpectedPAF = p_soft*PAF_soft + p_mod*PAF_mod + p_hard*PAF_hard`.
  Smooths abrupt class transitions without biological interpolation
  between SSDs.
