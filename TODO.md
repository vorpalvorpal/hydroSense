...
- **Update docs for the new imputation options.** The `imputation` vignette and
  any tutorial text predate `impute_method` (rescor_mi / cens / cens_factor) and
  `min_target_detect_freq`. Update the vignette to explain the three methods,
  when to prefer each (point accuracy vs calibrated/cheap uncertainty), and the
  target filter; refresh the `fit_imputation_model` examples; cross-check the
  README and `chronic-amspaf-interpretation` vignette for stale imputation
  descriptions.
