...
make_chem_k <- function(site, dates, mult = 1, seed = 1L, analytes = NULL) {
  ...
  purrr::map_dfr(dates, function(d) {
    vals <- vapply(analytes, function(a) {
      if (a %in% names(base)) {
        exp(stats::rnorm(1, log(base[[a]]), 0.35)) * mult
      } else {
        switch(a,
          pH = stats::runif(1, 6.5, 8),
          DOC = stats::runif(1, 1, 5),
          hardness = stats::runif(1, 20, 60),
          Ca = stats::runif(1, 4, 12),
          Mg = stats::runif(1, 2, 8),
          stats::runif(1, 1, 5)
        )
      }
    }, numeric(1))
...
test_that("I2: point-mode centre stable; summary centre is draw median", {
  ...
  m <- dplyr::inner_join(
    summ[, c("date", "median_ignorable")], ref,
    by = "date"
  )
  expect_equal(m$median_ignorable, m$m, tolerance = 1e-8)
})
...
test_that("I3: CI width is smaller near grab dates than mid-gap", {
  ...
  out$width <- out$hi_ignorable - out$lo_ignorable
  dist_to_grab <- vapply(out$date, function(d) {
    min(abs(as.numeric(d - .kf$dates)))
  }, numeric(1))
...
test_that("I7: a short-sampled analyte's modelled rows are clipped to its span", {
  ...
  tm <- .smw(fit_target_model(tgt_clip, .kf$rm,
    conc_units = "ug/L",
    min_obs_model = 10L,
...
test_that("I8: bounds are non-negative/ordered and the envelope is asymmetric", {
  ...
  expect_true(all(out$lo_ignorable >= 0, na.rm = TRUE))
  ordered_ok <- out$lo_ignorable <= out$median_ignorable &
    out$median_ignorable <= out$hi_ignorable
