...
test_that("default summary is geom_mean", {
  prep <- prepare_reference(make_ref())
...
test_that("geom_mean ≤ arith_mean for positive concentrations (AM-GM)", {
  ref <- make_ref()
  gm  <- prepare_reference(ref, summary = "geom_mean")
  am  <- prepare_reference(ref, summary = "arith_mean")
...
test_that("median ≤ p80 ≤ p90 ≤ p95", {
  ...
  med  <- prepare_reference(ref, summary = "median")
  p80  <- prepare_reference(ref, summary = "p80")
  p90  <- prepare_reference(ref, summary = "p90")
  p95  <- prepare_reference(ref, summary = "p95")
...
test_that("BDL observations contribute 0 to the summary", {
  ...
  prep <- suppressMessages(prepare_reference(ref))
