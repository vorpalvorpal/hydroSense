...
.bf <- local({
  ...
  rm <- tryCatch(
    fit_reference_model(ref, hydro = hydro, conc_units = "ug/L",
                        min_obs_model = 10L,
...
describe(".summarise_bracket()", {
  ...
  it("bracket mode returns both envelopes + precautionary composite", {
    ...
    expect_equal(out$lo_ignorable,
                 stats::quantile(c(1, 2, 3, 4, 5), lo_p, names = FALSE))
    expect_equal(out$hi_ignorable,
    ...
    expect_equal(out$lo_informative,
                 stats::quantile(c(2, 2, 3, 4, 4), lo_p, names = FALSE))
    expect_equal(out$hi_informative,
  ...
  it("ignorable mode returns only the ignorable columns (no informative, no precautionary)", {
    out <- leachatetools:::.summarise_bracket(
      toy, interval = 0.8, central = "median", gap_uncertainty = "ignorable")
  ...
  it("informative mode returns only the informative columns", {
    out <- leachatetools:::.summarise_bracket(
      toy, interval = 0.8, central = "median", gap_uncertainty = "informative")
  ...
  it("central='mean' uses the per-day draw mean for the centre", {
    out <- leachatetools:::.summarise_bracket(
      toy, interval = 0.8, central = "mean", gap_uncertainty = "bracket")
...
describe("amspaf_daily(gap_uncertainty=)", {
  ...
  it("ignorable summary exposes only ignorable envelope columns", {
    ...
    expect_true(all(c("deterministic", "median_ignorable",
                      "lo_ignorable", "hi_ignorable") %in% names(out)))
  ...
  it("informative summary exposes only informative envelope columns", {
    ...
    expect_true(all(c("deterministic", "median_informative",
                      "lo_informative", "hi_informative") %in% names(out)))
  ...
  it("bracket draws mode carries amspaf_ignorable and amspaf_informative per draw", {
    ...
    expect_true(all(c("date", "site_id", "draw_id",
                      "amspaf_ignorable", "amspaf_informative") %in% names(out)))
    counts <- out |>
  ...
  it("the deterministic column equals the point-mode (ndraws=NULL) estimate", {
    ...
    j <- dplyr::inner_join(
      ...
      by = c("date", "site_id"))
    expect_equal(j$deterministic, j$amspaf_pt, tolerance = 1e-6,
  ...
  it("the ignorable envelope equals the quantiles/median of its own draws", {
    skip_if(is.null(.bf$rm), "Reference model not fitted")
    interval <- 0.9; lo_p <- 0.05; hi_p <- 0.95
    summ <- run_daily(ndraws = 30L, seed = 5L, return = "summary",
  ...
  it("ignorable draws are byte-identical whether or not the informative path runs", {
    skip_if(is.null(.bf$rm), "Reference model not fitted")
    br <- run_daily(ndraws = 8L, seed = 13L, return = "draws",
                    gap_uncertainty = "bracket") |>
      dplyr::arrange(.data$site_id, .data$date, .data$draw_id)
    ig <- run_daily(ndraws = 8L, seed = 13L, return = "draws",
                    gap_uncertainty = "ignorable") |>
      dplyr::arrange(.data$site_id, .data$date, .data$draw_id)
    expect_equal(br$amspaf_ignorable, ig$amspaf_ignorable, tolerance = 1e-12,
  ...
  it("the informative band nests inside the ignorable band", {
    skip_if(is.null(.bf$rm), "Reference model not fitted")
    out <- run_daily(ndraws = 40L, seed = 7L, return = "summary",
                     interval = 0.9)
    tol <- 1e-6
  ...
  it("the two envelopes coincide at observation days and diverge across gaps", {
    ...
    expect_lt(mean(med_gap[is_grab], na.rm = TRUE),
  ...
  it("the bracket collapses (informative == ignorable) under dense daily sampling", {
    ...
    out <- suppressWarnings(suppressMessages(amspaf_daily(
      tgt_dense, reference_model = .bf$rm, interpolation = "model",
  ...
  it("central tendency is ordered deterministic ≤ informative ≤ ignorable (median)", {
    skip_if(is.null(.bf$rm), "Reference model not fitted")
    out <- run_daily(ndraws = 60L, seed = 9L, return = "summary",
                     central = "median")
    tol <- 1e-6
    # Exact for the mean (Jensen on the convex combine); expected for the median.
    # Assert on the aggregate plus a strong majority of days.
    expect_gte(mean(out$median_informative - out$deterministic), -tol)
    expect_gte(mean(out$median_ignorable - out$median_informative), -tol)
    expect_gt(mean(out$deterministic <= out$median_informative + tol), 0.8)
    expect_gt(mean(out$median_informative <= out$median_ignorable + tol), 0.8)
  })
  ...
  it("warns about ignorable gap treatment for rainfall hydrology + uncertainty", {
    ...
    w <- capture_warns(amspaf_daily(
      .bf$tgt, reference_model = .bf$rm, interpolation = "model",
      require_temperature = FALSE, conc_units = "ug/L",
  ...
  it("point mode (ndraws=NULL) ignores gap_uncertainty and returns the point schema", {
    ...
    expect_false(any(grepl("informative|ignorable|precautionary|deterministic",
                           names(out))))
  })
  ...
  it("a single tox analyte still yields a defined informative envelope", {
    ...
    out <- suppressWarnings(suppressMessages(amspaf_daily(
      tgt1, reference_model = .bf$rm, interpolation = "model",
