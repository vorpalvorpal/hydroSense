...
**hydroSense** analyses water quality in freshwater systems potentially
...
- **Multi-substance toxicity (msPAF).** Species sensitivity
  ...
  subtracts the *local* background via the Added Risk Approach. The
  background can be a static reference or a *contemporaneous* one
  ([`fit_reference_model()`](https://vorpalvorpal.github.io/hydroSense/reference/fit_reference_model.md))
  that predicts what the reference site would have shown at each
  sample’s moment from hydrology and season; a season-blind companion
  ([`fit_target_model()`](https://vorpalvorpal.github.io/hydroSense/reference/fit_target_model.md))
  predicts the impacted site between grabs, so
  [`mspaf_daily()`](https://vorpalvorpal.github.io/hydroSense/reference/mspaf_daily.md)
  can return a continuous daily risk series.
