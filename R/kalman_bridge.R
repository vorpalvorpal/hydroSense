...
#' [mspaf_daily()] (`interpolation = "model"`) are calibrated on **your** data.
#' For each modelled analyte it holds out each grab anchor (or a run of `block`
#' consecutive anchors), refits the OU/Kalman smoother on the rest, and checks
#' whether the held-out value lands inside the nominal `interval` predictive
#' band. The result is reported **per analyte and per impact tier**, because the
#' two tiers are usually calibrated differently:
#'
#' * **`"model"`** analytes (enough detected anchors to fit a hydrology impact
#'   model) are normally close to nominal.
#' * **`"bridge"`** analytes (too sparse for their own fit, interpolated from a
#'   shared shape) tend to be **under-covered** — their bands are too narrow.
#'
#' Use this to *measure*, then act with [mspaf_daily()]'s tier-aware `ou_scale`:
#' if the pooled `bridge` coverage is below your `interval`, raise the bridge
#' tier's scale (e.g. `ou_scale = c(model = 1, bridge = 2)`) and re-measure. See
#' `vignette("daily-uncertainty")` for the full workflow.
#'
#' Coverage is a finite-sample estimate; with few anchors (especially in the
#' bridge tier) it is noisy, so prefer **pooling across multiple sites** before
#' choosing a scale.
#'
#' @param target_model A fitted [fit_target_model()] object (class
#'   `"target_model"`).
#' @param interval Nominal coverage to test, in `(0, 1)`. Default `0.9`.
#' @param block Number of consecutive anchors held out per fold (default `1`).
#'   `block >= 2` probes mid-gap width rather than just the near-anchor pinch.
#' @param scale OU process-variance scale used while refitting, matching the
#'   `ou_scale` argument of [mspaf_daily()] (a single number here). Sweep it to
#'   find the value that brings a tier to nominal coverage.
#' @return A tibble with one row per analyte (`analyte`, `tier`, `n` = number of
#'   anchors, `coverage`, `mean_width`) plus a final `"(pooled)"` row
#'   (anchor-weighted). `coverage`/`mean_width` are `NA` for analytes with too
#'   few anchors to refit.
#' @seealso [fit_target_model()], [mspaf_daily()]
#' @examples
#' \dontrun{
...
loo_anchor_coverage <- function(target_model, interval = 0.9, block = 1L,
                                scale = 1) {
  if (!inherits(target_model, "target_model")) {
    cli::cli_abort(
      "{.arg target_model} must be a {.cls target_model} from \\
       {.fn fit_target_model}."
    )
  }
  checkmate::assert_number(interval, lower = 0, upper = 1)
  checkmate::assert_count(block, positive = TRUE)
  checkmate::assert_number(scale, lower = 0, finite = TRUE)
  .loo_anchor_coverage(target_model, interval = interval, block = block,
                       scale = scale)
}
